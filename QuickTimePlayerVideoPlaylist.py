#!/usr/bin/env python3
"""
QuickTime Player Video Playlist - Enhanced video playlist with AirPlay support
Features: Repeat One/All, Shuffle, Save/Load playlist, Drag reorder, Delete items
Based on QuickTimePlayerAudioPlaylist but for video files
"""
...
class VideoPlaylistPro(QMainWindow):
    def __init__(self):
        super().__init__()
        self.playlist = []
        ...
        self.settings_file = Path.home() / '.video_playlist_pro_settings.json'
        self.settings = {}
        self.load_settings()
    ...
    def init_ui(self):
        self.setWindowTitle("QuickTime Player Video Playlist")
        ...
        self.current_track_label = QLabel("No video playing")
        ...
        # File controls
        add_files_btn = QPushButton("Add Videos")
        ...
        self.play_one_btn.setToolTip("Play selected video only")
        ...
        # Status bar
        self.status_label = QLabel("0 videos")
    ...
    def add_files(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "Select Video Files",
            "",
            "Video Files (*.mp4 *.mov *.avi *.mkv *.m4v *.wmv *.flv *.webm);;All Files (*.*)"
        )
    ...
    def add_folder(self):
        ...
        if folder:
            ...
            if files_added > 0:
                self.update_status()
                self.generate_shuffle_queue()
                QMessageBox.information(self, "Success", f"Added {files_added} video files")
            else:
                QMessageBox.warning(self, "No Files", "No video files found in the selected folder")
    ...
    def clear_playlist(self):
        ...
        if self.playlist:
            ...
            if reply == QMessageBox.Yes:
                ...
                self.update_status()
                self.current_track_label.setText("No video playing")
    
    def save_playlist(self):
        """Save playlist to file"""
        if not self.playlist:
            QMessageBox.warning(self, "Empty Playlist", "No videos to save")
            return
    ...
    def load_playlist(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Success", f"Loaded {len(self.playlist)} videos")
            except Exception as e:
    ...
    def update_status(self):
        """Update status label"""
        track_text = f"{len(self.playlist)} video{'s' if len(self.playlist) != 1 else ''}"
...
def main():
    ...
    app.setApplicationName("QuickTime Player Video Playlist")
    
    window = VideoPlaylistPro()
    window.show()
